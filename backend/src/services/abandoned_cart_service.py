...
class AbandonedCartAgentService:
    ...
    async def get_abandoned_cart_agents(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for agent in all_agents:
                ...
                if template_info and template_info.get("template_slug") == self.target_template_slug:
            ...
            for agent in agents:
                ...
                for provider_slug, integration_data in integrations.items():
                    if integration_data.get("enabled", False):
                        ...
                        connection_result = await self.integration_service.test_integration_connection(
                            agent_id, provider_slug
                        )
                        
                        processed_integrations[provider_slug] = {
                            **integration_data,
                            "connection_test": connection_result.get("connection_result", {}),
                            "ready_for_abandoned_cart": connection_result.get("success", False)
                        }
                ...
                processed_agent = {
                    **agent,
                    ...
                    "ready_integrations_count": sum(
                        1 for integration in processed_integrations.values() 
    ...
    def generate_mock_abandoned_cart_data(self, provider_slug: str, company_info: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        mock_customers = [
            {
                ...
                "first_name": ["John", "Jane", "Mike", "Sarah", "David"][i % 5],
                "last_name": ["Smith", "Johnson", "Brown", "Davis", "Wilson"][i % 5],
        ...
        for i, customer in enumerate(mock_customers):
            ...
            abandoned_cart = {
                ...
                "currency": "USD",
                "abandoned_at": f"2025-09-{20 + i}T{10 + i}:30:00Z",
                "recovery_attempts": 0,
            ...
            mock_abandoned_carts.append(abandoned_cart)
        ...
        return {
            ...
            "total_recovery_value": sum(cart["total_value"] for cart in mock_abandoned_carts),
            "generated_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
            "mock_data": True
        }
    
    async def fetch_abandoned_cart_data(self, agent_id: str, provider_slug: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Generate mock data
            mock_data = self.generate_mock_abandoned_cart_data(provider_slug, company_info)
        ...
        try:
            ...
            for provider_slug, integration_data in integrations.items():
                if integration_data.get("enabled", False):
                    ...
                    cart_data_result = await self.fetch_abandoned_cart_data(agent_id, provider_slug)
                    
                    if cart_data_result["success"]:
                        platform_data[provider_slug] = cart_data_result["data"]
                    else:
                        print(f"   ❌ Failed to fetch data from {provider_slug}: {cart_data_result['message']}")
            
            # Create comprehensive payload
            payload = {
                "agent": {
                    "id": agent_info["agent_id"],
                    "name": agent_info["agent_info"].get("custom_name", "Abandoned Cart Agent"),
                    "type": "abandoned_cart_recovery",
                    "template_slug": self.target_template_slug,
                    "is_configured": agent_info["agent_info"].get("is_configured", False),
                    "total_interactions": agent_info["agent_info"].get("total_interactions", 0)
                },
                "company": {
                    "id": agent_info.get("company_info", {}).get("company_id"),
                    "name": agent_info.get("company_info", {}).get("company_name"),
                    "business_category": agent_info.get("company_info", {}).get("business_category"),
                    "phone_number": agent_info.get("company_info", {}).get("phone_number"),
                    "website": agent_info.get("company_info", {}).get("website"),
                    "timezone": agent_info.get("company_info", {}).get("timezone", "UTC")
                },
            ...
            print(f"✅ Created payload with {payload['summary']['total_abandoned_carts']} abandoned carts from {payload['summary']['total_platforms']} platforms")
    ...
    async def send_to_external_api(self, payload: Dict[str, Any], api_url: str, api_key: Optional[str] = None) -> Dict[str, Any]:
        """
