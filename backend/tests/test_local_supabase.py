...
class TestLocalSupabase:
    ...
    def test_database_operations(self, service_client: Client):
        ...
        try:
            ...
            # Insert
            insert_response = service_client.table('profiles').insert(test_profile).execute()
            assert len(insert_response.data) > 0
            print("✅ Database insert successful")
            
            # Select
            select_response = service_client.table('profiles').select('*').eq('username', 'dbtestuser').execute()
            assert len(select_response.data) > 0
            assert select_response.data[0]['username'] == 'dbtestuser'
            print("✅ Database select successful")
            
            # Update
            update_response = service_client.table('profiles').update({
                'full_name': 'Updated DB Test User'
            }).eq('username', 'dbtestuser').execute()
            assert len(update_response.data) > 0
            print("✅ Database update successful")
            
            # Delete (cleanup)
            service_client.table('profiles').delete().eq('username', 'dbtestuser').execute()
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            login_response = await client.post(
                "http://localhost:8000/api/v1/auth/supabase/login",
                json=login_data
            )
