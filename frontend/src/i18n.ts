...
export default getRequestConfig(async ({ locale }) => {
  ...
  if (!locales.includes(selectedLocale as any)) {
    console.error(`Invalid locale: ${selectedLocale}. Available locales: ${locales.join(', ')}`);
  ...
  try {
    console.log(`DEBUG - Attempting to load messages for locale: ${selectedLocale}`);
    const messages = (await import(`./messages/${selectedLocale}.json`)).default;
    console.log(`DEBUG - Successfully loaded messages for locale: ${selectedLocale}`);
    return {
      messages,
      locale: selectedLocale,
    };
  } catch (error) {
    console.error(`Error loading messages for locale ${selectedLocale}:`, error);
